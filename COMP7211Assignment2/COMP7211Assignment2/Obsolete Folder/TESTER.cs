...
namespace COMP7211Assignment2.Controller_Folder
{
    internal class TESTER
    {
        ...
        public static readonly string[] randomWordsArray = randomWords.Split(' ');
        ...
        public static async void RandomUsers(int amount)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                users.Add(new User(randomWordsArray[rnd.Next(randomWordsArray.Length - 1)], randomWordsArray[rnd.Next(randomWordsArray.Length - 1)], rnd.Next(10000000, 100000000), temppass, randomBool, item));
            }
            await PageData.PManager.FBHelper.firebase.Child("Students").PutAsync(users);
        }
    }
